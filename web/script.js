            ...
            let html = `
                <div class="announcements-header">
                    ...
                    <div class="announcements-actions">
                        <button onclick="showCreateAnnouncementModal()" class="btn-success">âž• Create Announcement</button>
                        <button onclick="loadAnnouncements()" class="btn-primary">ðŸ”„ Refresh</button>
                    </div>
                </div>
                    ...
                    <div class="announcement-card">
                        <div class="announcement-header">
                            <div class="announcement-title">
                                <span class="announcement-type">ðŸ“¢</span>
                                <h3>${announcement.title || 'Untitled Announcement'}</h3>
                                ${announcement.is_pinned ? '<span class="pinned-badge">ðŸ“Œ Pinned</span>' : ''}
                            </div>
                            <div class="announcement-actions">
                                <button onclick="editAnnouncement('${announcement.announcement_id}')" class="btn-small">âœï¸ Edit</button>
                                <button onclick="deleteAnnouncement('${announcement.announcement_id}')" class="btn-small btn-danger">ðŸ—‘ï¸ Delete</button>
                            </div>
                        </div>
                        ...
                        <div class="announcement-meta">
                            <span>ðŸ“ ${channelName}</span>
                            <span>ðŸ‘¤ By ${createdBy}</span>
                            <span>ðŸ•’ ${createdDate}</span>
                            ${announcement.message_id ? `<a href="https://discord.com/channels/${currentServerId}/${announcement.channel_id}/${announcement.message_id}" target="_blank">ðŸ”— View in Discord</a>` : ''}
                        </div>
                    </div>
                `;
            });
                ...
                html += `
                    <div class="embed-card">
                        ...
                        <div class="embed-actions">
                            <button onclick="editEmbed('${embed.embed_id}')" class="btn-small btn-primary">âœï¸ Edit</button>
                            <button onclick="sendEmbed('${embed.embed_id}')" class="btn-small btn-success">ðŸ“¤ Send</button>
                            <button onclick="deleteEmbed('${embed.embed_id}')" class="btn-small btn-danger">ðŸ—‘ï¸ Delete</button>
                        </div>
                    </div>
                `;
            });
            ...
            data.items.forEach(item => {
                html += `
                    <div class="shop-card">
                        ...
                        <div class="stock">Stock: ${item.stock === -1 ? 'âˆž' : item.stock}</div>
                        <div class="card-actions">
                            <button onclick="editShopItem('${item.item_id}')" class="btn-small btn-primary">âœï¸ Edit</button>
                            <button onclick="deleteShopItem('${item.item_id}')" class="btn-small btn-danger">ðŸ—‘ï¸ Delete</button>
                        </div>
                    </div>
                `;
            });
...
async function validateShopIntegrity() {
    ...
    try {
        ...
        let reportHtml = '';
        if (issues.length === 0) {
            reportHtml = '<div class="success-message">âœ… All shop items passed validation!</div>';
        } else {
            ...
            Object.values(data.tasks).forEach(task => {
                html += `
                    <div class="task-card">
                        ...
                        <div class="card-actions">
                            <button onclick="editTask('${task.task_id}')" class="btn-small btn-primary">âœï¸ Edit</button>
                            <button onclick="deleteTask('${task.task_id}')" class="btn-small btn-danger">ðŸ—‘ï¸ Delete</button>
                        </div>
                    </div>
                `;
            });
    ...
    try {
        ...
        if (data && data.announcements && data.announcements.length > 0) {
                ...
                html += `
                    <div class="announcement-card">
                        ...
                        <div class="announcement-meta">
                            ...
                            <span>ðŸ‘¤ ${createdBy}</span>
                            <span>ðŸ•’ ${createdDate}</span>
                            ${announcement.message_id ? `<a href="https://discord.com/channels/${currentServerId}/${announcement.channel_id}/${announcement.message_id}" target="_blank">ðŸ”— View</a>` : ''}
                        </div>
                    </div>
                `;
            });
            ...
            content.innerHTML = `
                <div class="empty-state">
                    <h3>No announcements yet</h3>
                    <button onclick="showCreateAnnouncementModal()" class="btn-success">âž• Create First Announcement</button>
                </div>
            `;
        }
    } catch (error) {
...
async function saveChannelSetting(type) {
    ...
    try {
        ...
        document.getElementById(statusId).textContent = 'âœ“ Saved';
        ...
        document.getElementById(statusId).textContent = 'âœ— Failed';
...
async function saveCurrencySettings() {
    ...
    try {
        ...
        document.getElementById('currency-settings-status').textContent = 'âœ“ Saved';
...
async function saveBotBehavior() {
    ...
    try {
        ...
        document.getElementById('bot-behavior-status').textContent = 'âœ“ Saved';
...
async function saveShopItem(event) {
    ...
    const itemData = {
        ...
        emoji: document.getElementById('shop-item-emoji').value || 'ðŸ›ï¸'
    };
...
/ /   M i s s i n g   f u n c t i o n s   r e s t o r a t i o n  
  
 a s y n c   f u n c t i o n   l o a d S e r v e r s ( )   {  
         t r y   {  
                 / /   T r y   / a p i / g u i l d s   f i r s t  
                 l e t   d a t a   =   n u l l ;  
                 t r y   {  
                         d a t a   =   a w a i t   a p i C a l l ( ' / a p i / g u i l d s ' ) ;  
                 }   c a t c h   ( e )   {  
                         c o n s o l e . l o g ( ' F a i l e d   / a p i / g u i l d s ,   t r y i n g   / a p i / u s e r / g u i l d s ' ) ;  
                         t r y   {  
                                 d a t a   =   a w a i t   a p i C a l l ( ' / a p i / u s e r / g u i l d s ' ) ;  
                         }   c a t c h   ( e 2 )   {  
                                 c o n s o l e . l o g ( ' F a i l e d   / a p i / u s e r / g u i l d s ' ) ;  
                                 r e t u r n ;  
                         }  
                 }  
  
                 c o n s t   s e l e c t   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' s e r v e r - s e l e c t ' ) ;  
                 i f   ( ! s e l e c t )   r e t u r n ;  
  
                 s e l e c t . i n n e r H T M L   =   ' < o p t i o n   v a l u e = " " > S e l e c t   S e r v e r < / o p t i o n > ' ;  
  
                 c o n s t   g u i l d s   =   d a t a . g u i l d s   | |   d a t a ;  
                 i f   ( g u i l d s   & &   A r r a y . i s A r r a y ( g u i l d s ) )   {  
                         g u i l d s . f o r E a c h ( g u i l d   = >   {  
                                 c o n s t   o p t i o n   =   d o c u m e n t . c r e a t e E l e m e n t ( ' o p t i o n ' ) ;  
                                 o p t i o n . v a l u e   =   g u i l d . i d   | |   g u i l d . g u i l d _ i d ;  
                                 o p t i o n . t e x t C o n t e n t   =   g u i l d . n a m e ;  
                                 s e l e c t . a p p e n d C h i l d ( o p t i o n ) ;  
                         } ) ;  
  
                         / /   S e l e c t   f i r s t   s e r v e r   i f   a v a i l a b l e  
                         i f   ( g u i l d s . l e n g t h   >   0 )   {  
                                 s e l e c t . v a l u e   =   g u i l d s [ 0 ] . i d   | |   g u i l d s [ 0 ] . g u i l d _ i d ;  
                                 o n S e r v e r C h a n g e ( ) ;  
                         }  
                 }  
         }   c a t c h   ( e r r o r )   {  
                 c o n s o l e . e r r o r ( ' F a i l e d   t o   l o a d   s e r v e r s : ' ,   e r r o r ) ;  
                 s h o w N o t i f i c a t i o n ( ' F a i l e d   t o   l o a d   s e r v e r s ' ,   ' e r r o r ' ) ;  
         }  
 }  
  
 f u n c t i o n   o n S e r v e r C h a n g e ( )   {  
         c o n s t   s e l e c t   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' s e r v e r - s e l e c t ' ) ;  
         i f   ( ! s e l e c t )   r e t u r n ;  
         c u r r e n t S e r v e r I d   =   s e l e c t . v a l u e ;  
         w i n d o w . c u r r e n t S e r v e r I d   =   c u r r e n t S e r v e r I d ;   / /   S y n c   g l o b a l  
  
         i f   ( c u r r e n t S e r v e r I d )   {  
                 / /   R e f r e s h   c u r r e n t   t a b  
                 c o n s t   a c t i v e T a b   =   d o c u m e n t . q u e r y S e l e c t o r ( ' . t a b - c o n t e n t . a c t i v e ' ) ;  
                 i f   ( a c t i v e T a b )   {  
                         c o n s t   t a b I d   =   a c t i v e T a b . i d ;  
                         s h o w T a b ( t a b I d ) ;  
                 }   e l s e   {  
...
 }  
  
 f u n c t i o n   s h o w T a b ( t a b I d )   {  
         / /   H i d e   a l l   t a b s  
         d o c u m e n t . q u e r y S e l e c t o r A l l ( ' . t a b - c o n t e n t ' ) . f o r E a c h ( t a b   = >   {  
                 t a b . c l a s s L i s t . r e m o v e ( ' a c t i v e ' ) ;  
                 t a b . s t y l e . d i s p l a y   =   ' n o n e ' ;  
         } ) ;  
  
         / /   D e a c t i v a t e   a l l   b u t t o n s  
         d o c u m e n t . q u e r y S e l e c t o r A l l ( ' . t a b - b u t t o n ' ) . f o r E a c h ( b t n   = >   {  
                 b t n . c l a s s L i s t . r e m o v e ( ' a c t i v e ' ) ;  
         } ) ;  
  
         / /   S h o w   s e l e c t e d   t a b  
         c o n s t   s e l e c t e d T a b   =   d o c u m e n t . g e t E l e m e n t B y I d ( t a b I d ) ;  
         i f   ( s e l e c t e d T a b )   {  
                 s e l e c t e d T a b . c l a s s L i s t . a d d ( ' a c t i v e ' ) ;  
                 s e l e c t e d T a b . s t y l e . d i s p l a y   =   ' b l o c k ' ;  
         }  
  
         / /   A c t i v a t e   b u t t o n  
         c o n s t   s e l e c t e d B t n   =   d o c u m e n t . q u e r y S e l e c t o r ( ` . t a b - b u t t o n [ d a t a - t a b = " $ { t a b I d } " ] ` ) ;  
         i f   ( s e l e c t e d B t n )   {  
                 s e l e c t e d B t n . c l a s s L i s t . a d d ( ' a c t i v e ' ) ;  
         }  
  
         / /   L o a d   t a b   c o n t e n t  
         i f   ( ! c u r r e n t S e r v e r I d )   r e t u r n ;  
  
         s w i t c h   ( t a b I d )   {  
                 c a s e   ' d a s h b o a r d ' :   i f   ( t y p e o f   l o a d D a s h b o a r d   = = =   ' f u n c t i o n ' )   l o a d D a s h b o a r d ( ) ;   b r e a k ;  
                 c a s e   ' u s e r s ' :   i f   ( t y p e o f   l o a d U s e r s   = = =   ' f u n c t i o n ' )   l o a d U s e r s ( ) ;   b r e a k ;  
                 c a s e   ' s h o p ' :   i f   ( t y p e o f   l o a d S h o p   = = =   ' f u n c t i o n ' )   l o a d S h o p ( ) ;   b r e a k ;  
                 c a s e   ' t a s k s ' :   i f   ( t y p e o f   l o a d T a s k s   = = =   ' f u n c t i o n ' )   l o a d T a s k s ( ) ;   b r e a k ;  
                 c a s e   ' a n n o u n c e m e n t s ' :   i f   ( t y p e o f   l o a d A n n o u n c e m e n t s   = = =   ' f u n c t i o n ' )   l o a d A n n o u n c e m e n t s ( ) ;   b r e a k ;  
                 c a s e   ' e m b e d s ' :   i f   ( t y p e o f   l o a d E m b e d s   = = =   ' f u n c t i o n ' )   l o a d E m b e d s ( ) ;   b r e a k ;  
                 c a s e   ' t r a n s a c t i o n s ' :   i f   ( t y p e o f   l o a d T r a n s a c t i o n s   = = =   ' f u n c t i o n ' )   l o a d T r a n s a c t i o n s ( ) ;   b r e a k ;  
                 c a s e   ' s e r v e r - s e t t i n g s ' :   i f   ( t y p e o f   l o a d S e r v e r S e t t i n g s T a b   = = =   ' f u n c t i o n ' )   l o a d S e r v e r S e t t i n g s T a b ( ) ;   b r e a k ;  
                 c a s e   ' p e r m i s s i o n s ' :   i f   ( t y p e o f   l o a d P e r m i s s i o n s T a b   = = =   ' f u n c t i o n ' )   l o a d P e r m i s s i o n s T a b ( ) ;   b r e a k ;  
                 c a s e   ' l o g s ' :   i f   ( t y p e o f   l o a d L o g s   = = =   ' f u n c t i o n ' )   l o a d L o g s ( ) ;   b r e a k ;  
                 c a s e   ' c o n f i g ' :   i f   ( t y p e o f   l o a d S e r v e r S e t t i n g s   = = =   ' f u n c t i o n ' )   l o a d S e r v e r S e t t i n g s ( ) ;   b r e a k ;  
         }  
 }  
  
 / /   I n i t i a l i z e  
...
         / /   C h e c k   a u t h  
         / /   W e   a s s u m e   a p i C a l l   h a n d l e s   4 0 1   r e d i r e c t  
         l o a d S e r v e r s ( ) ;  
 } ) ;  
 